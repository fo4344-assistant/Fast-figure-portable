      ...
      function auditApp(context = "manual") {
        let issues = [],
      ...
      function renderAssetTreeActionMenu(nodeType = null, context = null) {
        let trigger = $("assetActions"),
          ...
          definitions = (ASSET_TREE_ACTION_SCHEMA[nodeType] || []).filter(
            (definition) => !definition.when || definition.when(context),
          );
        ...
        if (!definitions.length) {
          ...
          definitions.forEach((definition) => {
            ...
            if (disabled && definition.disabledTitle) item.title = definition.disabledTitle;
            item.addEventListener("click", (event) => {
              event.preventDefault();
              event.stopPropagation();
              if (item.disabled) return;
              closeAssetTreeActionMenus();
              ASSET_TREE_ACTION_HANDLERS[definition.key]?.(context);
            });
            menu.append(item);
          });
        }
      ...
      function syncAssetTreeActionMenu() {
        ...
        if (!match || match.kind !== appFSM.state.assetSelection)
          return renderAssetTreeActionMenu();
        ...
        renderAssetTreeActionMenu(match.kind, context);
      }
      ...
      function configFromForm() {
        ...
        let previousAxes = previous.editor.globalSettings?.axes || {},
          ...
          editor = {
            ...
            lineWidth: Math.max(0.1, Math.min(20, Number($("lineWidth").value) || 2)),
            lineDash: $("lineDash").value || "solid",
            markerSymbol: $("markerSymbol").value || "circle",
            markerSize: Math.max(1, Math.min(40, Number($("markerSize").value) || 6)),
            barOpacity: Math.max(0.05, Math.min(1, Number($("barOpacity").value) || 1)),
            barLineWidth: Math.max(0, Math.min(10, Number($("barLineWidth").value) || 0)),
            xDivide: 1,
            yDivide: 1,
      ...
      function editChart(id) {
        ...
        let csv =
          getProjectCsv(c.editor?.objects?.[0]?.csvId) ||
          (c.editor?.editable === false ? ensureDefaultCsv() : null);
      ...
      function selectGraphObject(index) {
        ...
        let next = selectedObjectIndex === index ? null : index;
      ...
      function renderGraphObjects(objects = []) {
        ...
        box.querySelectorAll(".graph-object[draggable]").forEach((item) => {
          ...
          item.ondrop = (e) => {
            ...
            let next = [...objects],
              moved = next.splice(dragging, 1)[0];
            next.splice(to, 0, moved);
            if (selectedObjectIndex === dragging) selectedObjectIndex = to;
            else if (dragging < selectedObjectIndex && selectedObjectIndex <= to)
              selectedObjectIndex--;
            else if (to <= selectedObjectIndex && selectedObjectIndex < dragging)
              selectedObjectIndex++;
            appFSM.send("SELECT_GRAPH_OBJECT", {
              index: selectedObjectIndex,
              direction: "fsm-to-model",
            });
            saveGraphObjects(next);
          };
          item.querySelector(".graph-object-color").oninput = (e) => {
            let next = [...objects];
            next[+item.dataset.index] = { ...next[+item.dataset.index], color: e.target.value };
            saveGraphObjects(next);
          };
          item.querySelector(".graph-object-delete").onclick = (e) => {
            e.stopPropagation();
            saveGraphObjects(objects.filter((_, index) => index !== +item.dataset.index));
            appFSM.send("CLEAR_GRAPH_OBJECT", {
              index: null,
              direction: "fsm-to-model",
            });
      ...
      $("addGraphObject").onclick = () => {
        let target = getSelectedSlot();
        if (!target || !activeDataReady) return status("그래프 슬롯과 데이터를 먼저 선택하세요.");
        if (!target.chart) {
          let csv = getProjectCsv(activeCsvId);
          if (target.contentType === "image" || !csv)
          ...
          appFSM.send("SLOT_DATA_CONNECTED", payload);
          if (!payload.chart) return status("빈 슬롯에 그래프를 추가하지 못했습니다.");
          editChart(payload.chart.id);
          ...
          renderGraphObjects(payload.chart.editor.objects);
          updateGraphView();
          return;
        }
        let c = configFromForm(),
          object = {
            ...
            yAxisSide: c.editor.yAxisSide,
            type: c.editor.type,
            color: DEFAULT_COLORS[(editing?.editor?.objects?.length || 0) % DEFAULT_COLORS.length],
            legendName: c.editor.legendName,
            lineWidth: c.editor.lineWidth,
            lineDash: c.editor.lineDash,
            markerSymbol: c.editor.markerSymbol,
        ...
        c.editor.objects = [...ensureGraphObjects(editing || c), object];
        let xKey = axisKey("x", c.editor.xAxisSide),
          yKey = axisKey("y", c.editor.yAxisSide);
        if (!c.editor.globalSettings.axes[xKey].title) {
          c.editor.globalSettings.axes[xKey].title = c.editor.x;
          $(`${xKey}Title`).value = c.editor.x;
        }
        if (!c.editor.globalSettings.axes[yKey].title) {
          c.editor.globalSettings.axes[yKey].title = c.editor.y;
          $(`${yKey}Title`).value = c.editor.y;
        }
        c.id = target.chart;
        let payload = {
          slotId: target.id,
        ...
        appFSM.send("CHART_MODEL_REPLACED", payload);
        editing = payload.chart;
        selectedObjectIndex = c.editor.objects.length - 1;
        appFSM.send("SELECT_GRAPH_OBJECT", {
          index: selectedObjectIndex,
          direction: "fsm-to-model",
        });
        renderGraphObjects(c.editor.objects);
        updateGraphView();
      };
      function updateTypeSpecificOptions() {
        let type = $("chartType").value,
          showLine = ["scatter", "lines+markers", "hidden"].includes(type),
          showMarker = ["markers", "lines+markers"].includes(type),
          showBar = type === "bar";
      ...
      function renderLabelPreview() {
        let label = $("labelPreviewLabel"),
          slot = $("labelPreview").querySelector(".label-preview-slot"),
          geometry = labelPreviewGeometry();
        label.textContent = displayedSlotIdentifier(0);
        ...
        label.style.left = `${geometry.left + activeProject.labelSettings.x * geometry.scale}px`;
        label.style.top = `${geometry.top + activeProject.labelSettings.y * geometry.scale}px`;
        label.style.fontFamily = activeProject.labelSettings.fontFamily;
        label.style.fontSize = `${activeProject.labelSettings.fontSize * geometry.scale}px`;
        label.style.minWidth = `${20 * geometry.scale}px`;
        label.style.padding = `${2 * geometry.scale}px ${6 * geometry.scale}px`;
        $("labelPositionX").value = Number(activeProject.labelSettings.x.toFixed(2));
        $("labelPositionY").value = Number(activeProject.labelSettings.y.toFixed(2));
      }
      function applyLabelSettings() {
        activeProject.labelSettings.format = $("labelFormat").value;
        activeProject.labelSettings.parentheses = $("labelParentheses").dataset.active === "true";
        activeProject.labelSettings.order = $("labelOrder").value;
        activeProject.labelSettings.fontFamily = $("labelFontFamily").value;
        let fontSize = Number($("labelFontSize").value);
        activeProject.labelSettings.fontSize = Number.isFinite(fontSize)
          ? Math.max(6, fontSize)
          : 14;
        $("labelFontSize").value = activeProject.labelSettings.fontSize;
        renderLabelPreview();
        renderDashboard();
        schedulePlotResize();
      ...
      function applyLabelPosition() {
        activeProject.labelSettings.x = finiteLabelPosition(Number($("labelPositionX").value));
        activeProject.labelSettings.y = finiteLabelPosition(Number($("labelPositionY").value));
        renderLabelPreview();
        renderDashboard();
        schedulePlotResize();
        debugLog("annotation:label-position", { x: activeProject.labelSettings.x, y: activeProject.labelSettings.y });
        appFSM.notify("labels", "LABEL_POSITION_CHANGED");
      }
      function installLabelSettings() {
        ...
        $("labelPositionReset").onclick = () => {
          activeProject.labelSettings.x = 0;
          activeProject.labelSettings.y = 0;
          renderLabelPreview();
          renderDashboard();
          schedulePlotResize();
          debugLog("annotation:label-position-reset", { x: 0, y: 0 });
          appFSM.notify("labels", "LABEL_POSITION_RESET");
        };
        let preview = $("labelPreview"),
      ...
      $("resetGraphPalette").onclick = () => {
        ...
        saveGraphObjects(
          ensureGraphObjects(chart).map((object, index) => ({
            ...object,
      ...
      $("saveGraphPalette").onclick = () => {
        ...
        let blob = new Blob([JSON.stringify({ colors: graphPalette() }, null, 2)], {
            type: "application/json",
          }),
          a = document.createElement("a"),
          stamp = new Date().toISOString().replace(/[:.]/g, "-");
        a.href = URL.createObjectURL(blob);
        a.download = `chart-palette-${stamp}.json`;
        a.click();
      ...
      $("loadGraphPaletteFile").onchange = async (e) => {
        ...
        try {
          ...
          saveGraphObjects(
            ensureGraphObjects(chart).map((object, index) => ({
              ...object,
              color: valid[index % valid.length],
            })),
          );
